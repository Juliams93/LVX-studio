...
for (const f of readdirSync(root)) {
 if (/\.(css|js|png|webp|jpg|svg|ico)$/.test(f)||f==='index.html'||f==='projects') cpSync(join(root,f),join(out,f),{recursive:true});
}
...
const urls=[{path:'/'},{path:'/blog'},...posts.map(p=>({path:'/blog/'+p.slug,modified:p.modified}))];
